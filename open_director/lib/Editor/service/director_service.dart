...
import 'package:open_director/Editor/service/director/layer_player.dart';
...
class DirectorService {
  ...
  setProjectFirstTime(Project _project) async {
    isEntering = true;
    _position.add(0);
    _selected.add(Selected(-1, -1));
    editingTextAsset = null;
    ...
    // const sampleText = 'Dart is\nan object\noriented\nlanguage';
    final sampleText = data.text;
    const splitter = LineSplitter();
    final sampleTextLines = splitter.convert(sampleText);
    sampleTextLines.forEach((element) {
      addTextMaually(element.toString());
    });
    //Check project details edit function not to call addTextManually on updating details
  }
  ...
  addTextMaually(String text) async {
    if (isOperating) return;
    isAdding = true;
    editingTextAsset = Asset(
        type: AssetType.text,
  ...
  generateVideo(
      List<Layer> layers, VideoResolution videoResolution, BuildContext context,
      {int framerate}) async {
    if (filesNotExist) {
      _filesNotExist.add(true);
      return false;
    ...
    String outputFile =
        await generator.generateVideoAll(layers, videoResolution, context);
    if (outputFile != null) {
